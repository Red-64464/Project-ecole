<!-- Statistiques d'un éditeur (vue partagée par l'éditeur lui-même
     et par le répartiteur consultant les stats d'autrui).
     L'identité est lue dans l'URL (/stats/editor/:id). -->
<script setup>
import { ref, onMounted, watch } from "vue";
import { useRoute } from "vue-router";
import { getEditorStats, getEditorCurrent } from "@/services/statsService";
import { getEditorById } from "@/services/editorService";
...
async function load() {
  ...
  const person = await getEditorById(id);
  if (!person) {
    notify(`Éditeur ${id} introuvable`, "error");
    loading.value = false;
    return;
  }
  editor.value = person;

  current.value = await getEditorCurrent(id);
  history.value = await getEditorStats(
    id,
...
<template>
  ...
  <div v-else-if="editor" class="editor-stats">
    <h3>
      Pour l'éditeur :
      {{ editor.firstname }} {{ editor.lastname }} ({{ editor.id }})
    </h3>
    ...
    <section class="block">
      ...
      <table class="stats-table">
        <thead>
          <tr><th>Textes en cours</th><th>Mots à éditer</th></tr>
        </thead>
        <tbody>
    ...
    <section class="block">
      ...
      <table class="stats-table">
        <thead>
          <tr><th>Attribués</th><th>Édités</th><th>Dont rendus en retard</th><th>Mots</th></tr>
        </thead>
        <tbody>
          <tr>
